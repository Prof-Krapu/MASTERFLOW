...
| Teaching | mode raccordé au shell actif ; `TeachingWorkspaceSurface` partagé entre runtime et Lab | données runtime réelles dans `/ui-reset` ; fixtures explicites et séparées dans `/ui-lab` | `connected_verified` côté câblage, conformité visuelle encore `audit_required` | l'ancien constat « aucune page rendue » est superseded ; la revue MALEX reste distincte du câblage | vérifier couleurs, profils, classes, sujets, progression et 390 px sans redéclarer la page absente |
