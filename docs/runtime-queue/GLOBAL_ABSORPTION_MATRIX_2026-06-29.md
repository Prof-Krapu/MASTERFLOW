...
| Security Fabric | `contract_ready_pr_176` + `partial_runtime` | frontière de confiance V1, RAG anti-injection, action engine, hard stop, permissions | ban automatique, kill switch global, self-debug, provider/live | `SECURITY-FABRIC-002` garde déterministe partagé |
