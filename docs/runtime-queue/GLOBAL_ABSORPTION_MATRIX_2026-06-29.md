...
| Trust Fabric | `contract_ready_local` + `partial_runtime` | quatre dimensions séparées, trust RAG, confidence fields, audit, coûts et cockpit | score moral, moyenne globale, surveillance opaque, décisions automatiques | `TRUST-FABRIC-002` read model sans table |
