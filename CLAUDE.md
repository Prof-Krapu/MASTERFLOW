...
**Décisions de périmètre actées (sync MALEX/Vincent ; Q6 validée humainement le 2026-06-07)** : `user_runtime_loadout`, validation inbox dédiée et endpoints lourds (`/da`, `/assets`, `/inventory`, `/subjects`) = **hors V1** ; backflow/factories = `out_of_scope` ; **Owner Ops strict** — `owner_ops_private_diagnostic` reste privé/propriétaire, **jamais exposé dans l'UI, même en rôle `godmode`** ; `godmode` = surface console/diagnostic (lecture) qui **ne court-circuite pas** le pipeline de validation (`teacher+` sur `/actions/:id/validate`) ; rien de ces surfaces pour teacher/student. Le cloisonnement strict de la 1re carte est **maintenu** ; le brouillon IA « godmode étendu » est **annulé**. Détail daté dans `SUIVI.md` et `SYNC_THREAD_MALEX_VINCENT.md`.
