...
const RULES: Rule[] = [
  {
    ...
    route: {target: 'owner_cockpit', object_type: 'control_state', decision_authority: 'owner', decider_role: 'owner', reason: 'Le hard stop runtime reste à implémenter.'},
    next: {kind: 'ask_malex_decision', label: 'Geler les actions sensibles manuellement', reason: 'Le modèle hard stop n’existe pas encore.', required_validation: true, forbidden_followups: ['auto_cancel', 'auto_reset']},
    missed: {expected_process: 'hard_stop', missing_runtime_piece: 'control_state_runtime', user_impact: 'La consigne stop n’invalide pas encore les actions.', suggested_queue_task: 'Implémenter hard stop et action expiry.', severity: 'high'},
    confidence: 0.95,
