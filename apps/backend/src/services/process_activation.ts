...
const RULES: Rule[] = [
  {
    ...
    route: {target: 'owner_cockpit', object_type: 'control_state', decision_authority: 'owner', decider_role: 'owner', reason: 'Le cockpit permet de prévisualiser puis de geler une sélection explicite ; aucun stop n’est appliqué depuis le texte brut.'},
    next: {kind: 'inspect_context', label: 'Prévisualiser, sélectionner puis confirmer les actions à geler', reason: 'La granularité sélectionnée est disponible ; chaque action reste décochée tant que l’owner ne la confirme pas.', required_validation: true, forbidden_followups: ['auto_apply_stop', 'auto_cancel', 'auto_reset']},
    missed: {expected_process: 'hard_stop', missing_runtime_piece: 'hard_stop_control_state_runtime', user_impact: 'Les actions sélectionnées peuvent être gelées, mais le stop ne persiste pas encore comme état empêchant de nouvelles actions.', suggested_queue_task: 'Auditer un état hard-stop persistant et sa reprise explicite sans automatiser depuis le texte brut.', severity: 'high'},
    confidence: 0.95,
