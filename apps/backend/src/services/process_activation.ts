...
const RULES: Rule[] = [
  {
    ...
    route: {target: 'owner_cockpit', object_type: 'control_state', decision_authority: 'owner', decider_role: 'owner', reason: 'Le cockpit permet un hard-stop explicite limité à l’owner et à la Room réelle.'},
    next: {kind: 'inspect_context', label: 'Prévisualiser les actions puis activer explicitement le stop de la Room', reason: 'Le texte brut reste diagnostic-only ; seul le bouton owner active l’état persistant.', required_validation: true, forbidden_followups: ['auto_apply_stop', 'auto_cancel', 'auto_reset']},
    confidence: 0.95,
  ...
  },
  {
    ...
    route: {target: 'action_queue', object_type: 'usage_learning_candidate', decision_authority: 'owner', decider_role: 'owner', reason: 'Les findings D12 sont auto-détectées et routées vers les candidats d\'apprentissage.'},
    next: {kind: 'route_to_validation_inbox', label: 'Consulter les findings auto-détectées', reason: 'La détection automatique est active via scan d\'événements. Les findings passent en revue avant usage.', required_validation: false, forbidden_followups: ['auto_patch', 'auto_canon']},
    missed: {expected_process: 'd12_auto_fix', missing_runtime_piece: 'automatic_recovery_or_escalation', user_impact: 'Les ratés sont détectés automatiquement, mais la correction reste manuelle.', suggested_queue_task: 'Implémenter un processus de récupération automatique pour les patterns validés.', severity: 'low'},
    confidence: 0.88,
    status: 'candidate_ready_for_review',
  },
];
