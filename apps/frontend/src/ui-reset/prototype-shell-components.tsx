...
export function PrototypeSystemChrome({
  ...
  const panelContent = (mobile: boolean): ReactNode => {
    ...
    if (renderedPanel === 'queue') {
      return mobile ? (
        <>
          <button className="proto-mobile-system-page__item" type="button"><LoaderCircle size={19} /><span><strong>Compilation du contexte</strong><small>68% terminé</small></span></button>
          <button className="proto-mobile-system-page__item" type="button"><History size={19} /><span><strong>Préparation des ressources</strong><small>En attente</small></span></button>
        </>
      ) : (
        <>
          <small>Queue</small>
          <strong>2 tâches en cours</strong>
          <p>Compilation du contexte · 68%</p>
          <p>Préparation des ressources · en attente</p>
        </>
      );
    }
    if (renderedPanel === 'notifications') {
      return mobile ? (
        <>
          <button className="proto-mobile-system-page__item" type="button"><Check size={19} /><span><strong>Une validation t’attend</strong><small>À examiner maintenant</small></span></button>
          <button className="proto-mobile-system-page__item" type="button"><Sparkles size={19} /><span><strong>Deux sorties disponibles</strong><small>Génération terminée</small></span></button>
        </>
      ) : (
        <>
          <small>Notifications</small>
          <strong>3 éléments à regarder</strong>
          <p>Une validation et deux sorties sont disponibles.</p>
        </>
      );
    }
    if (renderedPanel === 'dm') {
      return mobile ? (
        <button className="proto-mobile-system-page__item" type="button"><Send size={19} /><span><strong>Nouvelle requête</strong><small>Une réponse directe est attendue</small></span></button>
      ) : (
        <>
          <small>Messages privés</small>
          <strong>1 nouvelle requête</strong>
          <p>Une demande directe attend ta réponse.</p>
        </>
      );
    }
    ...
    <>
      <header className="proto-systembar">
        ...
        <div className="proto-systembar__actions">
          <button aria-expanded={panel === 'queue'} aria-label="Queue en cours" className="proto-system-action proto-system-action--queue" data-tooltip="Queue · 2 tâches" onClick={() => onTogglePanel('queue')} type="button">
            <LoaderCircle size={19} />
            <span className="proto-system-badge">2</span>
          </button>
          <button aria-expanded={panel === 'notifications'} aria-label="Notifications" className="proto-system-action proto-system-action--notifications" data-tooltip="Notifications" onClick={() => onTogglePanel('notifications')} type="button">
            <Bell size={19} />
            <span className="proto-system-dot">3</span>
          </button>
          <button aria-expanded={panel === 'dm'} aria-label="Messages privés et requêtes" className="proto-system-action proto-system-action--dm" data-tooltip="Messages privés" onClick={() => onTogglePanel('dm')} type="button">
            <Send size={19} />
            <span className="proto-system-dot">1</span>
          </button>
