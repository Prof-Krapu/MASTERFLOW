...
type LabSection = 'foundations' | 'dock' | 'forms' | 'states' | 'overlays' | 'home' | 'project' | 'navigation' | 'identity' | 'pipeline';
type LabViewport = 'desktop' | 'mobile';
type LabTheme = 'dark' | 'light';
type ProjectLabScenarioId = 'godmode-empty' | 'teacher-ready' | 'student-assigned' | 'student-forbidden' | 'archived-read-only' | 'mobile-390';
...
const labProject: Project = {
  project_id: 'project-lab-001',
  owner_id: 'owner-lab',
  name: 'Identité de lancement',
  status: 'active',
  visibility: 'private',
  created_at: Date.UTC(2026, 6, 28, 9, 0),
  updated_at: Date.UTC(2026, 7, 5, 9, 35),
};

const labArchivedProject: Project = {...labProject, project_id: 'project-lab-archived', name: 'Archive festival 2025', status: 'archived'};

const labProjectMembers: ProjectMember[] = [
  {project_id: labProject.project_id, user_id: 'owner-lab', role: 'owner', created_at: labProject.created_at},
  {project_id: labProject.project_id, user_id: 'editor-lab', role: 'editor', created_at: labProject.created_at},
  {project_id: labProject.project_id, user_id: 'student-lab-1', role: 'participant', created_at: labProject.created_at},
  {project_id: labProject.project_id, user_id: 'student-lab-2', role: 'participant', created_at: labProject.created_at},
];

const labProjectResources: Resource[] = [
...
const labAttachableResources: Resource[] = [
...
const labProjectCheckpoint: RoomCheckpoint = {
  checkpoint_id: 'checkpoint-lab-project',
  room_id: 'room-lab',
  room_instance_id: 'room-instance-lab',
  user_id: 'owner-lab',
  project_id: labProject.project_id,
  privacy_scope: 'private',
  reason: 'stable_activity',
  summary: 'Reprendre la sélection des sources de lancement.',
  active_widgets: [],
  active_mode: 'project',
  decisions: [],
  open_loops: ['Choisir la prochaine source'],
  media_queue_refs: [],
  asset_queue_refs: [],
  resource_refs: ['resource-lab-001'],
  next_recommended_action: 'Relire le socle de marque avant d’ajouter une nouvelle source.',
  rollback_light_possible: true,
  created_at: Date.UTC(2026, 7, 5, 9, 20),
};

const projectScenarioLabels: Record<ProjectLabScenarioId, string> = {
...
export function ComponentLab({workspaceId}: ComponentLabProps): ReactElement {
  ...
  const [projectScenarioId, setProjectScenarioId] = useState<ProjectLabScenarioId>('teacher-ready');
  const [labProjectName, setLabProjectName] = useState('');
  const [labProjectResourceId, setLabProjectResourceId] = useState('');
  ...
  const projectScenario = (() => {
    if (projectScenarioId === 'godmode-empty') {
      return {project: null, projects: [], members: [], resources: [], memberRole: null, canCreate: true, canAttach: false, checkpoint: null};
    }
    if (projectScenarioId === 'student-forbidden') {
      return {project: null, projects: [], members: [], resources: [], memberRole: null, canCreate: false, canAttach: false, checkpoint: null};
    }
    if (projectScenarioId === 'archived-read-only') {
      return {
        project: labArchivedProject,
        projects: [labArchivedProject],
        members: labProjectMembers.map((member) => ({...member, project_id: labArchivedProject.project_id})),
        resources: labProjectResources,
    ...
    const student = projectScenarioId === 'student-assigned';
    return {
      project: labProject,
      projects: [labProject],
      members: labProjectMembers,
      resources: labProjectResources,
      memberRole: student ? 'participant' as const : 'owner' as const,
      canCreate: !student,
      canAttach: !student,
      checkpoint: labProjectCheckpoint,
    };
  })();
  ...
  return (
    <main className={`component-lab is-${theme}`} style={previewStyle}>
      ...
      <section className={`component-lab__bench is-${viewport}`} aria-label={`Banc de test ${section}`}>
        ...
        {section === 'project' ? (
          <div className="component-lab__stage component-lab__stage--project">
            <label className="component-lab__scenario-select">
              <span>Scénario Project</span>
              <select
                onChange={(event) => {
                  const next = event.target.value as ProjectLabScenarioId;
                  setProjectScenarioId(next);
                  if (next === 'mobile-390') setViewport('mobile');
                }}
                value={projectScenarioId}
              >
                {(Object.entries(projectScenarioLabels) as Array<[ProjectLabScenarioId, string]>).map(([id, label]) => (
                  <option key={id} value={id}>{label}</option>
                ))}
              </select>
            </label>
            <ProjectWorkspaceV2
              attachableResources={labAttachableResources}
              canAttachResource={projectScenario.canAttach}
              canCreateProject={projectScenario.canCreate}
              checkpoint={projectScenario.checkpoint}
              createName={labProjectName}
              memberRole={projectScenario.memberRole}
              members={projectScenario.members}
              onAttachResource={() => undefined}
              onBackHome={() => setSection('home')}
              onCreateNameChange={setLabProjectName}
              onCreateProject={(event) => event.preventDefault()}
              onProjectChange={() => undefined}
              onResourceChange={setLabProjectResourceId}
              project={projectScenario.project}
              projects={projectScenario.projects}
              resources={projectScenario.resources}
              selectedProjectId={projectScenario.project?.project_id ?? ''}
              selectedResourceId={labProjectResourceId}
              status="ready"
              statusMessage="Fixture locale : aucune donnée ni permission réelle n’est modifiée."
            />
          </div>
        ) : null}
